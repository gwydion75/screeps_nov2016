    ...
    run: function(creep) {
        ...
        if (creep.memory.walling) {
            var repairwall = creep.room.find(FIND_STRUCTURES, { 
                filter: (structure) => { 
                    return (structure.structureType == STRUCTURE_RAMPART && structure.hits < 10000 && structure.hits > 0 || structure.hits < 10000 && structure.hits > 0 && structure.structureType == STRUCTURE_WALL)
                }
            });
